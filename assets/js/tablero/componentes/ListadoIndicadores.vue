<template>
    ...
    <b-form-group  :label=" $t('_clic_indicador_para_agregarlo_tablero_') "  v-if="indicadores.length > 0">
        <div style="max-height: 40vh; min-height: 70px; overflow: auto;">
            <b-input-group class="mt-3" >
                <b-input-group-text slot="prepend">
                    <font-awesome-icon icon="search" />
                </b-input-group-text>
                <b-form-input autocomplete="off" :placeholder=" $t('_buscar_') " v-model="filtro"> </b-form-input>
            </b-input-group>
            
            <b-list-group>
                <b-list-group-item v-for=" (ind, k) in indicadoresFiltrados"  :key="k" @click.prevent="agregarIndicador(ind)"
    ...
    export default {
        ...
        methods : {
            agregarIndicador( indicador ){
                ...
                let index = (this.$store.state.indicadores.length == 0) ?  0 : Math.max.apply(Math, this.$store.state.indicadores.map(function(o) { return o.index })) + 1;
                indicador.filtro = '';
                indicador.orden = '';
