<template>
    ...
    <table class="table table-bordered table-striped" data-cols-width="70,100" style="max-width:15.5cm">
        <thead>
            ...
            <tr class="sonata-ba-view-title" bgcolor='#E1EFFB' >
                <th data-f-bold="true" data-fill-color="FFE1EFFB" >{{ $t( '_nombre_campo_' )}}</th>
                <th data-f-bold="true" data-fill-color="FFE1EFFB" >{{ $t( '_descripcion_' )}}</th>
            </tr>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    <ul class="list-group">
                        <li class="list-group-item" v-for="(v, k) in indicador.ficha.clasificacion_tecnica" :key="k">
                            {{ v.descripcion }}
                        </li>
                    </ul>
                </td>
            ...
            <tr>
                ...
                <td>{{ (indicador.ficha.periodo != null) ? indicador.ficha.periodo.descripcion : '' }}</td>
            </tr>
            <tr>
            ...
            <tr>
                <th colspan="2" >
                    ...
                    <table class="table table-bordered table-striped"
                        v-for="( v, k) in indicador.ficha.variables" :key="k">
                        <caption style="caption-side: top">Var # {{ k + 1 }}</caption>
                        <tbody>

                            <tr>
                                <th width="20%" data-f-bold="true">{{ $t( '_confiabilidad_' ) }}</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <ul class="list-group">
                                        <li class="list-group-item" v-for="(v1, k1) in v.origen_dato.conexion" :key="k1">{{ v1.nombre }} ({{ v1.ip }})</li>
                                    </ul>
                                </td>
                            </tr>
                            <tr>
            ...
            <tr v-if="indicador.ficha.alertas.length > 0">
                <th colspan="2">
                    <table class="table table-bordered table-striped"  >
                        <caption style="caption-side: top">{{ $t( '_alertas_' )}}</caption>
                        <thead>
                            <tr class="sonata-ba-view-title" bgcolor='#E1EFFB'>
                        ...
                        <tbody>
                            <tr v-for="( v, k ) in indicador.ficha.alertas" :key="k">
                                <td :style=" { background: v.color.codigo }" :data-fill-color="getColorExceljs(v.color.codigo)">{{ v.color.color }} </td>
