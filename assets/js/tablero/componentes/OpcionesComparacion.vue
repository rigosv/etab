        ...
        <b-card no-body class="col-12">
            <b-tabs pills card>
                        ...
                        <b-form-group >
                            <template slot="label">
                                ...
                                <transition name="slide-fade">
                                    <b-button size='sm' variant="danger" v-if="indicador.configuracion.dimensionComparacion != '' "
                ...
                <b-tab>
                    ...
                    <b-card-text>
                        <div class="col-sm-12 " v-if="indicador.dataComparar.length > 0" style="padding: 20px;">
                            <span class="text-primary">{{ $t('_indicadores_agregados_') }}</span>

                            <b-list-group style="max-height: 40vh; overflow: auto;">
                                <b-list-group-item class="d-flex justify-content-between align-items-center"
                                                   v-for="(item, key) in indicador.dataComparar"  :key="key"
                                >
                                    <font-awesome-icon icon="check"  class="text-success" size="2x" ></font-awesome-icon> {{ item.nombre.toUpperCase() }}
                                    <b-badge variant="light">
                                        <b-button variant="outline-danger" :title=" $t('_borrar_') " @click="quitarComparacion(item)">
                        ...
                        <buscar v-model="filtroIndicadores" @buscar="buscarIndicadores" :enter="true" ref='buscarInput'></buscar>

                        <b-list-group style="max-height: 40vh; overflow: auto;">
                            <b-list-group-item class="d-flex justify-content-between align-items-center"
                                               v-for="(item, key) in indicadoresFiltrados"  :key="key"
                            >
                                {{ item.nombre.toUpperCase() }}
                                <b-badge variant="light" v-if="!indicador.configuracion.agregados.includes(item.id)">
                                    <b-button variant="outline-success" :title=" $t('_agregar_') " @click="agregarIndicador(item)">
...
<script>
    ...
    export default  {
        ...
        computed : {
            dimensionesComparar : function(){
                let vm = this;

                let dimensiones = this.indicador.dimensiones.filter( dimension => {
                    //Verificar  que no sea la dimensión actual
                    if ( dimension != vm.indicador.dimension ) {
                        //Verificar que no esté en los filtros
                        let esFiltro = false ;
                        vm.indicador.filtros.forEach(filtro => {
                ...
                return dimensiones.map( d => { return {text: this.indicador.informacion.dimensiones[d].descripcion, value: d} });
            },
        },
        ...
        methods : {
            buscarIndicadores : function () {
                if (this.filtroIndicadores.length >= 3 ){
                    let vm = this;
                    axios.get("/api/v1/tablero/listaIndicadores?tipo=busqueda&busqueda="+ this.filtroIndicadores )
            ...
            agregarIndicador:  function( indicadorC ) {
                ...
                let indicadorCompleto = {'id': indicadorC.id, 'dimensiones': dimensiones, 'nombre': indicadorC.nombre, 'data' : []};
                ...
                if ( existeD ) {
                    ...
                    axios.post("/api/v1/tablero/datosIndicador/" + indicadorC.id + "/" + vm.indicador.dimension, json)
                        .then(function (response) {
                            if ( response.data.status == 200 ){
                    ...
                    let dimensionesC = this.indicador.dimensiones.filter(x => dimensiones.includes(x));

                    if ( dimensionesC.length == 0 ){
                        this.$snotify.warning( this.$t("_indicadores_no_tienen_dimensiones_en_comun_") );
                    } else {
                        //Quitar la comparación por dimensión por si existe
                        this.indicador.configuracion.dimensionComparacion = ''
                        this.$snotify.warning( this.$t("_indicador_no_tiene_dimension_actual_") );
                        this.indicador.configuracion.agregados.push( indicadorC.id );
            ...
            quitarComparacion:  function( indicadorC ) {
                this.indicador.dataComparar = this.indicador.dataComparar.filter( ind => { return ind.id != indicadorC.id});
                this.indicador.configuracion.agregados = this.indicador.configuracion.agregados.filter( id => { return id != indicadorC.id});
            }
        },

        watch: {
            'indicador.configuracion.dimensionComparacion' : function () {
                ...
                if (! ['BARRA', 'BARRAS', 'COLUMNAS', 'COLUMNA', 'DISCRETEBARCHART', 'LINECHART', 'LINEA', 'LINEAS'].includes(this.indicador.configuracion.tipo_grafico.toUpperCase()) ) {
