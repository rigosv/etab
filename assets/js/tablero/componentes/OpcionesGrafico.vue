    ...
    <div class="container-fluid row" >
        <b-form-group :label="$t('_cambiar_dimension_')" class="col-12">
            ...
            <b-form-select v-model="dimension" @change="cambiarDimension">
                <option v-for="(item, key) in dimensionesFiltradas" :key="key" :value="item" > 
        ...
        <transition name="slide-fade">
            <b-form-group :label="$t('_cambiar_orden_')" class="col-12" v-if="puedeOrdenar">
                <b-row>
                    <b-col v-for=" (i, k) in seriesOrden "
                        :key="k"
        ...
        <b-form-group :label="$t('_seleccione_tipo_grafico_')" >
            <b-container fluid class="p-4 tipos_graficos bg-light" >
                <b-form-radio-group id="radio-group-2"  name="radio-sub-component" v-model="indicador.configuracion.tipo_grafico" >
                    <b-row class="mb-4">
                        <b-col md="3" 
                        ...
                        >
                            <b-form-radio :value="item.codigo" class="text-center" >
                                <b-img thumbnail fluid  :src="getGrafico(item.codigo)" :alt="item.descripcion" style="margin-bottom: 10px;"></b-img>
                                {{item.descripcion}}
                            </b-form-radio>
                        </b-col>                        
                    </b-row>
...
<script>
    ...
    export default  {
        ...
        computed : {
            comparaDimensiones : function() {
                return this.indicador.configuracion.dimensionComparacion != '';
            },
            comparaIndicadores : function() {
                return this.indicador.dataComparar.length > 0;
            },

            seriesOrden : function() {
                return (this.comparaIndicadores || this.comparaDimensiones ) ?  {'x': 'orden_x'} : {'x': 'orden_x', 'y': 'orden_y'};
            },

            dimensionesFiltradas : function(){
                let vm = this;
                ...
                this.indicador.dataComparar.map ( ind => {
                    dimensiones = dimensiones.filter(x => ind.dimensiones.includes(x));
                });
            ...
            tiposGraficos : function () {
                ...
                let tipos = this.indicador.informacion.dimensiones[this.indicador.dimension].graficos;
                
                if (this.comparaDimensiones || this.comparaIndicadores ) {
                    tipos = tipos.filter ( tipo => {                    
                        return !['PIECHART', 'PIE', 'PASTEL', 'TORTA','MAPA', 'GEOLOCATION', 'MAP'].includes(tipo.codigo.toUpperCase());
                    });
                    ...
                    if ( !tipos.find( tipo => { return ['LINECHART', 'LINEA', 'LINEAS'].includes(tipo.codigo.toUpperCase()) }) ) {
                        tipos.push({codigo:'lineas', descripcion : this.$t('_lineas_')});
                    }

                }
                
                return tipos.sort((a, b) => a.descripcion.localeCompare(b.descripcion) )
            },

            puedeOrdenar : function () {
                return ['BARRA', 'BURBUJA', 'BUBBLE', 'BARRAS', 'COLUMNAS', 'COLUMNA', 'DISCRETEBARCHART', 'LINECHART', 'LINEA', 'LINEAS'].includes(this.indicador.configuracion.tipo_grafico.toUpperCase());
            }
        },
        mounted : function() {
        ...
        methods : {
            ...
            cambiarOrden : function ( tipo, modo){
