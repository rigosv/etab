...
export default class GraficoMixin extends Vue {
    ...
    protected doubleClickTime:number = 0;
    private doubleClickThreshold:number = 500;
    private seleccionActiva:boolean =  false;
    get dec() {
        return  (isNaN( this.indicador.ficha.cantidad_decimales) || this.indicador.ficha.cantidad_decimales == null ) ? 2 : this.indicador.ficha.cantidad_decimales;
    }
    ...
    get datosOrdenados():any {
        let datos_ = this.indicador.data.map((f: any) => {
