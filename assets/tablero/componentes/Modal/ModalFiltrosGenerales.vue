  ...
  >
    <form class="form-horizontal">
      <div class="form-group">
        ...
        <div class="col-sm-10">
          <b-form-select v-model="dimensionGeneral" @change="recuperarValoresDimensionGeneral">
            <option v-for="(item, key) in dimensionesGenerales" :key="key" :value="item" > 
              {{ item.descripcion }}
            </option>
          </b-form-select>
        </div>
      ...
      <div
        class="form-group"
        v-show="!(filtroGeneralEsCatalogo) && dimensionGeneral != '' && dimensionGeneral != '?' "
      >
        <label for="valorFiltro1" class="col-sm-2 control-label">{{$t("_valor_filtro_")}}</label>
        <div class="col-sm-10">
      ...
      <div class="form-group" v-show="filtroGeneralEsCatalogo">
        <label for="valorFiltro2" class="col-sm-2 control-label">{{ $t("_valor_filtro_") }}</label>
        <div class="col-sm-10">
          <b-form-select v-model="valorFiltroGeneralCatalogo" id="valorFiltro2">
            <option v-for="(item, key) in datosCatalogo" :key="key" :value="item" > 
              {{ item.descripcion }}
            </option>
          </b-form-select>          
        </div>
      </div>
      <button class="btn btn-primary" type="button" @click="aplicarFiltroGeneral" > {{ $t('_aplicar_filtro_')  }} </button>
    </form>
  </b-modal>
</template>
...
export default class ModalFiltrosGenerales extends Mixins(IndicadorMixin) {
  ...
  private filtroGeneralEsCatalogo:boolean =  false;
  private dimensionesGenerales:string[] = [];
  private datosCatalogo:any[] = [];
  private valorFiltroGeneral:string = '';
  private valorFiltroGeneralCatalogo:any = {};
  
  
  public iniciarModal():void {
      let dimensionesExistentes:string[] = [];
      let dimensionesAux:any[] = [];
      
      let sala = this.$store.state.sala;
      
      // Cargar los datos de los indicadores de la sala
      for (let indicador of this.$store.state.indicadores) {
        let dims = indicador.informacion.dimensiones;
        for (let codigo of Object.keys(dims)) {
          if ( !dimensionesExistentes.includes(codigo) ) {
              dimensionesExistentes.push(codigo);
              dimensionesAux.push({'descripcion': dims[codigo].descripcion, 'codigo': codigo});
          }
        };
      };
      this.dimensionesGenerales = dimensionesAux.sort((a: any,b: any) => { return a.descripcion.localeCompare(b.descripcion) });
  ...
  public aplicarFiltroGeneral(): void {
      ...
      let existe = false;
      let etiqueta = '';
      let dimension = this.dimensionGeneral;
      let valor = ( this.filtroGeneralEsCatalogo ) ? this.valorFiltroGeneralCatalogo.descripcion : this.valorFiltroGeneral.trim();

      let index: number = 0;
      if ( dimension != '' && valor !='' ) {
          for (let ind of this.$store.state.indicadores) {
              nuevosFiltros = [];
              for (let filtro of ind.filtros) {
