...
export const store = new Vuex.Store({
    ...
    mutations: {
        setIndicadores(state, indicadores:object[] ): void {
            ...
            state.layout = state.indicadores.map( (ind:any) => { return ind.configuracion.layout });
        ...
        quitarIndicador ( state, index:number ): void {
            ...
            state.indicadores = state.indicadores.filter( (ind:any) => { return ind.index != index });
            state.indicadoresAllData = state.indicadoresAllData.filter( (ind:any) => { return ind.index != index });
